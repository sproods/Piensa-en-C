...
void main(void)
{
	...
	do
	{
		printf("Digite el número de filas de los arreglos: ");
		scanf("%d", &FIL);
	}
	while(FIL < 1 || FIL > MAX);
	...
	do
	{
		printf("Digite el número de columnas de los arreglos: ");
		scanf("%d", &COL);
	}
	while(COL < 1 || COL > MAX);
...
/* La función Suma se utiliza para sumar los arreglos y almacenar el resultadoj en un tercer arreglo bidimensional. */
{
	int I, J;
	for (I = 0; I < F; I++)
		for (J = 0; J < C; J++)
