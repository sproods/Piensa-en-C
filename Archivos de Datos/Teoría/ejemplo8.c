 ...
 * estructura tipo alumno, que se almacenará posteriormente en un archivo. */
{
    ...
    while (r)
    {
        ...
        /* Observa que la función fwrite tiene cuatro argumentos. El primero se utiliza
        para indicar la variable tipo estructura que se desea almacenar; el segundo muestra
        el tamaño de esa variable en términos de bytes; el tercero señala el número de variables
        que se leerán o almacenarán en el dispositivo de almacenamiento secundario, y el último
        representa el apuntador al inicio de la estructura FILE. */
