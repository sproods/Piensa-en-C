...
/* Incrementa salarios.
 * El programa incrementa el salario de los empleados de una empresa -actualiza el archivo correspondiente-
...
/* Esta función se utiliza para incrementar el salario de todos aquellos empleados que hayan tenido ventas
 * anuales por más de $1,000,000. Actualiza además el archivo correspondiente. */
{
    int i, j, t;
    float sum;
    empleado emple;
    ...
    {
        i = ftell(ap) / t;
        /* La función ftell se utiliza pra conocer la posición de nuestro apuntador en el archivo.
         * La variable i nos proporciona en este caso el tamaño de todos los bloques que existen debajo
         * de nuestra posición. Si conocemos el tamaño de cada bloque, entonces podemos obtener el
         * número de bloques que hay exactamente debajo de nuestra posición. */
        ...
        if (sum > 1000000)
        {
            ...
            /* Nos posicionamos para esctibir el registro actualizado. */

            fwrite(&emple, sizeof(empleado), 1, ap);
            fseek(ap, i * sizeof(empleado), 0);
            /* Nos posicionamos nuevamente para leer el siguiente registro.
             * Esta instrucción no debería ser necesaria, pero la función fwrite
             * se comporta a veces de manera inestable en algunos compiladores de C.
             * Para asegurarnos que siempre funcione correctamente, realizamos este
