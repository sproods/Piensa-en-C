...
typedef struct          // Declaración de la estructura vendedor.
{
    ...
    domicilio dom;      // domi es de tipo estructura domicilio.
    float sal;          // Salario mensual.
    fpago pago;         // pago es de tipo unión fpago.
    int cla;            // Clave forma de pago.
} vendedor;

void Lectura(vendedor *, int);      // Prototipo de funciones.
...
void main(void)
{
    ...
    do
    {
        printf("Digite el número de vendedores: ");
        scanf("%d", &TAM);
    }
...
/* Esta función se utiliza para generar las ventas totales anuales de cada uno de los
...
/* Esta función se utiliza para incrementar 5% el salario de todos aquellos vendedores
 * cuyas ventas anuales superaron $1,500,000. */
{
    int I, J;
    float SUM;
...
/* Esta función se utiliza para generar un listado de todos aquellos vendedores que en el
 * año vendieron menos de $300,000. */
{
    int I, J;
    float SUM;
    ...
    for (I = 0; I < T; I++)
    {
        ...
        if (SUM < 300000.00)
            printf("\nNúmero de empleado: %d\nNombre: %s\nVantas: %.2f", A[I].num, A[I].nom, SUM);
    }
}

void F4(vendedor A[], int T)
/* Esta función se usa para imprimir el número del empleado, el nombre del banco y el número
 * de cuenta de todos aquellos empleados a quienes deposita su sueldo en cuenta de cheques. */
{
    int I;
    float SUM;
