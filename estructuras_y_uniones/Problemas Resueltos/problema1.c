...
/* Comercializadora farmaceútica.
 * El programa maneja infomación sobre ventas, inventario, reabastecimiento y nuevos
 * productos de una comercializadora farmaceútica. */
...
void main(void)
{
    ...
    do
    {
        printf("Digite el número de productos: ");
        scanf("%d", &TAM);
    }
    while (TAM > 100 || TAM < 1);
    ...
    while (OPE)
    {
        switch (OPE)
        {
            ...
            case 2: Reabastecimiento(INV, TAM);
                break;
            case 3: Nuevos_Productos(INV, &TAM);    // Se pasa el parámetro por referencia, porque se puede modificar el número de elementos del arreglo en la función.
                break;
...
/* Esta función se utiliza para leer un arreglo unidimensional de tipo estructura producto de T elementos. */
{
    ...
    for (I = 0; I < T; I++)
    {
        printf("\nDigite información del producto %d", I + 1);
        printf("\n\tClave: ");
        scanf("%d", &A[I].clave);
        fflush(stdin);
...
/* Esta función se utiliza para manejar las ventas a un cliente. Se ingresan productos
 * y cantidades, el fin de datos está dado por el cero. Además de obtener el total de
 * las ventas, se actualiza el inventario. */
{
    int CLA, CAN, I, RES;
    float TOT, PAR;

    printf("\nDigite la clave del producto -0 para salir- : ");
    scanf("%d", &CLA);
    ...
    while (CLA)
    {
        ...
        if ((I == T) || (A[I].clave > CLA))
            ...
            {
                printf("\nNo existe en el inventario la cantidad solicitada. Solo hay %d", A[I].existencia);
                printf("\nLos lleva 1 - Sí  0 - No?: ");
                scanf("%d", &RES);
        ...
        printf("\nDigite la siguiente clave del producto -0 para salir- : ");
        scanf("%d", &CLA);
    }

    printf("\nTotal de ventas: %f", TOT);
}
...
/* Esta función se utiliza para reabastecer al inventario. */
{
    ...
    while (CLA)
    {
        ...
        printf("\nDigite otra clave del producto -0 para salir- : ");
        scanf("%d", &CLA);
    }
}
 ...
 * Dado que los productos se encuentran ordenados por clave, puede suceder que al
 * insertar un nuevo producto haya que mover los elementos del arreglo para que
 * continúen ordenados. */
{
    int CLA, I, J;
    ...
    while ((*T < 30) && (CLA))
    {
        ...
        if (I == *T)        // Se inserta el elemento en la última posición.
        {
            ...
            *T = *T + 1;
            ...
            printf("\nDigite otra clave de producto -0 para salir- : ");
            scanf("%d", &CLA);
    }
...
/* Esta función se utiliza para escribir la información almacenada en el inventario,
 * un arreglo unidimensional de tipo estructura producto de T elementos. */
{
    int I;
